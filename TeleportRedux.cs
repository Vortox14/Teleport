namespace TeleportRedux;
public class Teleport : SonsMod
{
    private SContainerOptions container;
    private readonly SLabelOptions[] text = new SLabelOptions[30];
    private readonly List<PlayerNameUiLink> _players = new();
    private bool _tp = false;
    private int _offset = 0;
    private float _timer = 0;
    private readonly int listCount = 7;

    protected override void OnInitializeMod()
    {
        TeleportReduxConfig.TeleportReduxConfiger.Init();
    }
    ...
    protected override void OnSdkInitialized()
    {
        ...
        // The line below is needed to allow the mod settings menu to be opened within RedLoaders settings menu.
        SettingsRegistry.CreateSettings(this, null, typeof(TeleportReduxConfig.TeleportReduxConfiger));
    }
    ...
    private void Update()
    {
        if (LocalPlayer.IsInWorld && !PauseMenu.IsActive && !TheForest.UI.Multiplayer.ChatBox.IsChatOpen && !DebugConsole.Instance._showConsole)
        {
            ...
            if (Input.GetKeyDown(TeleportReduxConfig.TeleportReduxConfiger.InterfaceKey.Value))
            {
                if (!_tp)
                    CheckTP();
                else
            ...
            int _inputInt = Input.inputString.Length > 0 ? Input.inputString[0] - '0' : -1;
